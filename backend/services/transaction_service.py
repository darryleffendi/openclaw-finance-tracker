...
def insert_transaction(
    ...
    with get_connection() as conn:
        ...
        if acct:
            ...
            account_repo.update_balance(conn, category, delta)
    ...
    with get_connection() as conn:
        ...
        if row["category"] == "salary" and row["type"] == "income":
            for dr in transaction_repo.find_salary_distributions(conn, row["date"]):
                ...
                account_repo.update_balance(conn, dr["category"], -dr["amount"])
        ...
        if row["note"] != "auto-distribution from salary":
            ...
            account_repo.update_balance(conn, row["category"], -delta)
